...
export default function Orders({ type }) {
    ...
    const fetchJobDetail = async () => {
      try {
        const response = await axios.get(
          ...
          }/api/admin/getJobDetailsByState/${id}?state=${type}`,
          {
            headers: {
        ...
        console.error(`[Orders] Error fetching job detail:`, error);
      }
    };
    fetchJobDetail();
  }, [id, type]);
  ...
  const handleSendToBidding = async () => {
    try {
      ...
      alert(response.data.message);
      window.location.href("/admin/jobs/active");
    } catch (error) {
      alert("Failed to send the job to bidding");
